...
def maintenance():
    ...
    if request.method == 'POST':
        ...
        left_apartments = root.left_list.traverse()
        right_apartments = root.right_list.traverse()

        for apartment in left_apartments + right_apartments:
            if apartment[6].lower() == status.lower():
...
def updation(block,house,occ):
    ...
    if not block.isdigit() or not house.isdigit() or not occ.isdigit():
    ...
    for row in l:
        if row[0] == str(block) and row[1] == str(house):
            row[3] = str(occ)
            flag = True
            break
    ...
    return "Updation successful" if flag else "Updation unsuccessful"'''
'''
    
@app.route('/update_occupancy', methods=['GET', 'POST'])
def updation():
    if request.method == 'POST':
        block = request.form.get('block')
        house = request.form.get('house')
        occ = request.form.get('occ')
        if block and block.isdigit() and house and house.isdigit() and occ:
            block = int(block)
            house = int(house)
            result = updation(block, house, occ)
        ...
        return redirect(url_for('update_occupancy'))
...
def updation():
    if request.method == 'POST':
        ...
        if flag:
            flash("Updation successful!!")
        else:
            flash("Updation unsuccessful!!")        
